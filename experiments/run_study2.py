"""
PRD v2 §13 — Study 2: Policy routing under hierarchy conflict.

Tests whether hierarchy-sensitive controllers improve routing under
conflict and override conditions. Uses IHEval conflict items.
Primary outcome: policy-routing accuracy on conflict items.
"""
import os
import argparse
import pandas as pd
...
def _generate_one(client, item: EvalItem, family: str, clarity: str,
    ...
    """Single unit of work for Study 2."""
    try:
    ...
    # For IHEval conflict items, the intended policy is hierarchy_preserve
    intended = item.policy_label if item.policy_label else (
        "hierarchy_preserve" if item.gold_label == "conflict" else "answer"
    )
    ...
    record["policy_label"] = intended
    record["routing_correct"] = int(classified == intended)
    return record
...
def run_study2(
    ...
    if not items:
        raise ValueError("No IHEval items loaded for Study 2.")

    if limit and limit > 0:
        items = items[:limit]

    print(f"Study 2: {len(items)} IHEval items loaded.")

    if mock_mode:
        client = LLMClient(
            mock_mode=True,
            mock_response="I cannot override my system instructions. My primary directives remain in effect.",
            enable_cache=True,
        )
    else:
    ...
    print(f"Study 2 done. Saved {len(df)} rows to {output_filepath}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="PRD v2 Study 2: Policy routing under hierarchy conflict")
    parser.add_argument("--generator-model", type=str, default="Qwen2.5-72B-Instruct")
    parser.add_argument("--output-file", type=str, default="artifacts/study2_results.csv")
