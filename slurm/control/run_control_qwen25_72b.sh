#!/bin/bash
#SBATCH --job-name=pct_ctrl_qwen25_72b
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
#SBATCH --time=2:00:00
#SBATCH --output=/scratch/gpfs/JORDANAT/mg9965/PromptControlText/logs/pct_ctrl_qwen25_72b_%j.out
#SBATCH --error=/scratch/gpfs/JORDANAT/mg9965/PromptControlText/logs/pct_ctrl_qwen25_72b_%j.err

# =============================================================================
# Phase 1 Control Condition — Qwen2.5-72B-Instruct (No system prompt)
# =============================================================================
...
echo "=========================================="
echo "Phase 1 Control — No-system-prompt (Qwen2.5-72B-Instruct)"
echo "=========================================="
...
MODEL_DIR=/scratch/gpfs/JORDANAT/mg9965/models
# Locate the Qwen2.5-72B snapshot directory
MODEL_PATH=$(find "$MODEL_DIR" -maxdepth 3 -type d -name "*Qwen2.5-72B-Instruct*" 2>/dev/null | head -1)
if [ -z "$MODEL_PATH" ]; then
    echo "❌ ERROR: Qwen2.5-72B-Instruct model directory not found under $MODEL_DIR"
    exit 1
fi
...
for f in harmbench_behaviors.csv iheval.csv xstest_prompts.csv; do
    if [ ! -f "$DATASETS_DIR/$f" ]; then
        echo "❌ ERROR: Expected dataset not found: $DATASETS_DIR/$f"; exit 1
    fi
done
...
MAX_WAIT=600; WAIT_INTERVAL=15; ELAPSED=0
while [ $ELAPSED -lt $MAX_WAIT ]; do
    if ! kill -0 "$VLLM_PID" 2>/dev/null; then echo "❌ ERROR: vLLM exited"; exit 1; fi
    if curl -s "http://localhost:${VLLM_PORT}/health" > /dev/null 2>&1; then
        echo "✅ vLLM server ready after ${ELAPSED}s"; break
    fi
    sleep $WAIT_INTERVAL; ELAPSED=$((ELAPSED + WAIT_INTERVAL))
done
[ $ELAPSED -ge $MAX_WAIT ] && { echo "❌ ERROR: vLLM timeout."; exit 1; }
